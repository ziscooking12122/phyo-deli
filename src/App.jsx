  ...
  useEffect(() => {
    ...
    signInAnonymously(auth)
      .then(() => {
        ...
        onSnapshot(storesCollection, (snapshot) => {
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
        {stores.map(store => (
          <div key={store.id} style={{ border: '1px solid #ccc', padding: '1rem', width: '250px' }}>
            <img src={store.image} alt={store.name} style={{ width: '100%', height: '150px', objectFit: 'cover' }} />
            <h3>{store.name}</h3>
            <p>{store.category}</p>
          </div>
        ))}
